...
class HomeController extends Controller
{
    public function dashboard()
    {
        ...
        $saldo_actual =
            Summary::whereType("add")
                ->whereStatus("PAID")
                ->whereDate("date", "<=", $now)
                ->sum("amount") -
            Summary::whereType("out")
                ->whereStatus("PAID")
                ->whereDate("date", "<=", $now)
                ->sum("amount");

        $ingresos = Summary::whereType("add")
            ->whereStatus("PAID")
            ->whereYear("date", $currentYear)
            ->sum("amount");
        $gastos = Summary::whereType("out")
            ->whereStatus("PAID")
            ->whereYear("date", $currentYear)
            ->sum("amount");

        // Month-over-month comparisons
        $ingresos_mes = Summary::whereType("add")
            ->whereStatus("PAID")
            ->whereYear("date", $now->year)
            ->whereMonth("date", $now->month)
            ->sum("amount");
        $ingresos_mes_anterior = Summary::whereType("add")
            ->whereStatus("PAID")
            ->whereYear("date", $lastMonth->year)
            ->whereMonth("date", $lastMonth->month)
            ->sum("amount");

        $gastos_mes = Summary::whereType("out")
            ->whereStatus("PAID")
            ->whereYear("date", $now->year)
            ->whereMonth("date", $now->month)
            ->sum("amount");
        $gastos_mes_anterior = Summary::whereType("out")
            ->whereStatus("PAID")
            ->whereYear("date", $lastMonth->year)
            ->whereMonth("date", $lastMonth->month)
            ->sum("amount");
