...
class ExportController extends Controller
{
    ...
    public function reportePdfMovimiento(Request $request)
    {
        ...
        if ($categoria_id != null) {
            $filter[] = ["category_id", "=", $categoria_id];
            $summary = Summary::where($filter)
                ->whereDate("date", "<=", $hoy)
                ->where("future", "=", 1);
        }
        if (isset($subcategorias)) {
            $filter[] = ["id_attr", "=", $subcategorias];
            $summary = Summary::where($filter)
                ->whereDate("date", "<=", $hoy)
                ->where("future", "=", 1);
        }
        ...
        if (isset($start) and isset($finish)) {
            ...
            $summary = Summary::whereBetween("date", [$start1, $finish1])
                ->where($filter)
                ->where("future", "=", 1);
        } else {
            if ($filter) {
                $summary = Summary::where("date", "=", $hoy)
                    ->where("future", "=", 1)
                    ->where($filter);
            } else {
                $summary = Summary::where("date", "=", $hoy)->where(
                    "future",
                    "=",
                    1,
                );
        ...
        $cuenta = Account::where("id", $cuenta_id)
            ->select("account_name")
            ->first();
        $categoria = Category::where("id", $categoria_id)
            ->select("name")
            ->first();
    ...
    public function reportePdfSocio(Request $request)
    {
        ...
        $sumaTotalPendiente = Detail::whereYear("date", $year)
            ->where("status", 0)
        ...
        $sumaTotalPendienteDolar = Detail::whereYear("date", $year)
            ->where("status", 0)
            ->where("student_id", $student_id)
        ...
        $movimientos = Detail::whereYear("date", $year)
            ->where("student_id", $student_id)
            ->whereStatus(1)
            ->whereBetween("date_paid", [$start, $finish])
            ->orderBy("date_paid", "desc")
            ->get();
        $sumaTotal =
            $movimientos->where("summary_type", "add")->sum("amount") -
            $movimientos->where("summary_type", "out")->sum("amount");
        $pendientes = Detail::whereYear("date", $year)
            ->whereStatus(0)
            ->where("student_id", $student_id)
            ->get();
        ...
        return $pdf->stream(
            "Reporte_Estudiante_" . $student->full_name . ".pdf",
        );
    }
